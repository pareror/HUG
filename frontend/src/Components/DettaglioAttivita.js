...
import { ArrowLeft, Calendar, Clock, MapPin, Users, User2, Euro, Trash2 } from "lucide-react";
import axios from "axios";
import { jwtDecode } from "jwt-decode";
import "../css/DettaglioAttivita.css";
import GestisciUtenzaModal from "./GestisciUtenzaModal";
import VisualizzaCaregiverModal from "./Pazienti/VisualizzaCaregiverModal";
...
function DettaglioAttivita() {
  ...
  // Carica i dettagli dell'attività (per attività interne, tipo "I")
  useEffect(() => {
    const fetchActivity = async () => {
      try {
        ...
        setVisibilita(response.data.activity.visibile ?? 0);
  ...
  // Se l'utente è un paziente, controlla se è iscritto all'attività e aggiorna lo stato isIscritto
  useEffect(() => {
    const checkIscrizione = async () => {
      try {
        ...
        setIsIscritto(response.data.isIscritto);
      } catch (err) {
  ...
  // Handler per toggle della visibilità (solo per centri)
  ...
  // Handler per visualizzare i caregiver (apre il modal in sola lettura)
  ...
  return (
    <div className="activity-detail">
      ...
      <div className="button-container">
        {role !== "paziente" ? (
          <>
            <button className="button button-primary" onClick={() => navigate(`/dashboard/attivita/interna/${id}/modifica`)}>
              Modifica Attività
            </button>
          ...
          <>
            ...
            <button className="button button-tertiary" onClick={() => setShowCaregiverModal(true)}>
